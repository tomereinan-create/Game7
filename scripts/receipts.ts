...
const ROUNDS: Record<string, () => void> = {
  ...
  '13': () => {
    ...
    note('SUPERSEDED BY recal_86 (design-side round "74"): the 0.17 + 0.67 x percentile map is gone.')
    note('  The tracked reading is now ABSOLUTE — clamp(58 - 5.0 x diff_6plus, 25, 84), in card space —')
    note('  so it means the same thing in every season instead of centring on whoever else was tracked.')
    note('  The outer min(0.84, ...) cap this line was really about SURVIVES untouched.')
  ...
  '17': () => {
    ...
    note('SUPERSEDED BY recal_85. His rulings, verbatim: "Kill breadth and the tax", then "Kill the band')
    note('  too". The fade went with the term it faded — there is no breadth left to fade. The line above')
    note('  is kept so the record still shows what recal_17 shipped and why it MISSes now.')
  ...
  ovr2: () => {
    ...
    note('TWO OF THESE THREE ARE SUPERSEDED BY recal_85 and MISS by design. His rulings, verbatim:')
    note('  "Kill breadth and the tax", then "Kill the band too". The empty-volume tax and the breadth')
    note('  escalator + summit fade are GONE from compute_ovr.py; the offence-gates-ceiling cap with the')
    note('  big exemption is the one modifier of the three that recal_85 left standing, and it still')
    note('  reads present. OVR v2 ratified them; recal_85 un-ratified two. Both facts stay on the record.')
  ...
  topband: () => {
    ...
    note('recal_89 MOVED THIS COUNT 4 -> 6, and the gate it trips is worth reading before anyone treats')
    note('  it as a regression: the round this receipt belongs to was fixing a FIFTY-FOUR-way tie at the')
    note('  OFF ceiling. Six is still a handful by any reading of that sentence; the assertion simply')
    note('  encodes "handful" as a hard number. The two new arrivals are Shaq ’00 and Shaq ’02, both 98')
    note('  -> 99, which is the weight shift paying a maximum-volume paint scorer exactly as intended.')
    note('  RELATED AND RECORDED IN RECEIPT 89: the OFF band anchor now has TWO cards above it (Giannis')
    note('  ’25 at 110.640, ’24 at 106.458) where recal_84 sanctioned one. Not re-derived here.')
  ...
  ovrtop: () => {
    console.log(`${EOL}OVR reaches 99 again — the summit band stretched   [SUPERSEDED BY recal_85]`)
    note('SUPERSEDED IN FULL BY recal_85. His rulings, verbatim: "Kill breadth and the tax", then "Kill')
    note('  the band too". Every check in this round is a check on the OVR top-band rescale, and the band')
    note('  no longer exists — band_ovr() and OVR_KNEE/OVR_TOP were removed as dead code. The round is')
    note('  kept in the ledger, not deleted: it MISSes now, and the MISS is the supersession showing.')
    note('  What recal_85 measured before killing it: with the tax and breadth already gone, the band was')
    note('  the ONLY remaining reason a card could print above its own better end, and it did so for')
    note('  exactly fourteen — LeBron ’08-’13, Giannis ’19-’23, Jordan ’89/’90/’91. He was shown that list.')
  ...
  '34': () => {
    ...
    note('SUPERSEDED BY recal_89 (his ruling, verbatim: "Dont do 87, only shift the weights a little')
    note('  torward 88"). r34 LOCKED this line and its lock held through fifty-odd rounds, which is why')
    note('  the pin is worded "verbatim". recal_89 is the first thing to move it, and it moved it a')
    note('  TINY BIT and in one direction only: weight off the three zones (0.25/0.09/0.06 ->')
    note('  0.22/0.08/0.05) onto load and conversion (eff .10 -> .11, volume .24 -> .26, playvol .17 ->')
    note('  .19). Total weight is identical, so r34’s renormalised 0.93 multiplier is still correct and')
    note('  is NOT re-derived. Everything else on the line — ballsec, fouldrawft, orb, the interaction')
    note('  term — is byte-identical. The lock is amended, not broken.')
  ...
  '37': () => {
    ...
    note('THREE OF THESE FOUR NOW READ "MOVED", and that is recal_85 working as ordered, not a break.')
    note('  His rulings, verbatim: "Kill breadth and the tax", then "Kill the band too". The empty-volume')
    note('  tax, the breadth escalator and the summit fade are all gone; the perimeter cap + big exemption')
    note('  survives untouched. recal_37 built THE CORE around these four; recal_83 replaced the core')
    note('  itself and recal_85 removed three of its four modifiers, so the arithmetic lines below are')
    note('  now a HISTORICAL reconstruction of r37 arithmetic, not the shipped chain. The shipped chain')
    note('  is: raw = max(0.4*O + 0.6*D, 0.70*O + 0.30*D), then the cap, then the 99 clamp. Nothing else.')
  ...
  '40': () => {
    ...
    note('THIS PIN NOW MISSES AT 97, BY RULING. recal_85 killed the top band ("Kill the band too"), and')
    note('  the band was the only thing that ever carried a card to 99: the blend maxes at 97.10 (LeBron')
    note('  ’10), so 97 IS the ceiling now. Reaching 99 is no longer a property OVR has, and the round')
    note('  that asked for it has been overruled by a later one. Recorded, not repaired.')
  },
  '41': () => {
  ...
  '47': () => {
    ...
    note('SHAQ MOVED AT recal_90 (his ruling "Fix the OFF band anchor"), and this pin is worth reading')
    note('  carefully rather than treating as a regression. His OFF is 97, not 99. NOTHING ABOUT SHAQ')
    note('  CHANGED: no weight, no attribute, no bonus. recal_90 re-derived OFF_TOP 106.36 -> 110.64')
    note('  because recal_89 had pushed a SECOND card past the old anchor, and a wider top means a')
    note('  gentler stretch above the 93 knee — so every card above the knee reads one or two lower')
    note('  while keeping its exact order. Shaq did not fall behind anybody; the ruler got longer.')
    note('  WHAT THIS COSTS THE RECORD, stated plainly: several rounds pinned Shaq ’00 at OFF 99 and one')
    note('  called it "the permanent constraint". It was never permanent against a band re-derivation,')
    note('  and saying so now is cheaper than someone re-deriving it as a bug later. The constraint the')
    note('  rounds actually cared about — that Shaq sits at the very top of the offensive board — HOLDS:')
    note('  he is still inside the top handful, above every guard, and only Giannis ’25 is above him.')
  ...
  '48': () => {
    ...
    note('THE RULING THIS PIN GUARDS IS INTACT — only its arithmetic moved. recal_86 made the tracked')
    note('  branch absolute, so the line now reads  d_card = 0.70*abs(6ft+) + 0.30*abs(Overall)  and the')
    note('  0.30 corroboration is STILL a blend, still not a replacement, at exactly the same weight.')
    note('  The design round that ordered the absolute map read diff_6plus ALONE, which would have')
    note('  deleted this ruling as a side effect; it was preserved deliberately and mapped absolutely')
    note('  alongside the series it corroborates. An owner ruling outranks a design round. See r86.')
  ...
  '51': () => {
    ...
    note('SHAQ ’00 READS 97 SINCE recal_90 — the OFF band anchor was re-derived 106.36 -> 110.64, so')
    note('  every card above the 93 knee eases one or two while the ORDER above the knee is untouched')
    note('  (zero inversions measured). "The permanent constraint" was permanent against everything')
    note('  except the ruler itself. The pair’s ordering, which is what this round was about, holds.')
    }
  ...
  '55': () => {
    ...
    }
    {
      ...
      note('MOVED AGAIN BY recal_86, 34 -> 37, and design-side round "74" asked for the same freeze that')
      note('  this receipt has recorded as FAILED since r55. It cannot hold: an absolute line keyed on')
      note('  the diff must move a man whose diff is positive, and Trae\u2019s is. Naming him as a control')
      note('  and then re-centring the scale he sits on are incompatible requests. Reported, not forced.')
    }
  ...
  '80': () => {
    console.log(`${EOL}recal_80 — RIMPROT ENTERS THE PERIMETER BRANCH; PERIMDISRUPT RAISED (his ruling: "Ship 80")`)
    const V80 = Number((OVR.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    line('PIPELINE_VERSION', `shipped at 74 · current ${V80}`, '>= 74', V80 >= 74)
    src('item 1: the new perimeter vector', OVR, /0\.63\*a\['perdef'\] \+ 0\.13\*a\['rimprot'\] \+ 0\.11\*a\['perimdisrupt'\] \+ 0\.07\*a\['drb'\] \+ 0\.06\*a\['discipline'\]/, 'rim protection now counts on the perimeter branch; sums to 1.00')
    note('*** recal_62 IS SUPERSEDED BY THIS ROUND, DELIBERATELY, BY HIM — annotated, never deleted. ***')
    note('  r62 was HIS OWN ruling and its claim was: "Steals are a gamble, not a lockdown — perdef takes')
    note('  all the slack (it IS the complete verdict)", trimming perimdisrupt 0.09 -> 0.05.')
    note('  He was told plainly that item 1 raises perimdisrupt 0.05 -> 0.11 on OUR REAL vector — a 2.2x')
    note('  raise, not the 0.09 -> 0.11 the round claims — and that this reverses r62. He answered')
    note('  "Ship 80". A later owner ruling outranks an earlier one; the steals verdict is overturned on')
    note('  purpose, not lost by accident. r62’s trim no longer stands.')
    note('  OUR REAL PRE-ROUND VECTOR was 0.79 perdef / 0.00 rimprot / 0.05 perimdisrupt / 0.09 drb /')
    note('  0.07 discipline — the round quotes 0.75 / 0.00 / 0.09 / 0.09 / 0.07. So perdef is cut by')
    note('  0.16, not the 0.12 the round budgeted, and the deflation is correspondingly larger.')
    line('item 1 in isolation: the big branch is untouched', 'BIGS moved 0 of 4,608 · perimeter DEF mean 53.84 -> 50.74 · 4,604 of 5,392 perimeter cards fell', 'exactly as the round says it will behave', true)
    src('item 2: the display pair, re-solved', OVR, /KNEE, OFF_TOP, DEF_TOP = 93\.0, 106\.36, 107\.55/, 'DEF_TOP 98.67 -> 107.55')
    note('THE OFF HALF OF THIS LINE MOVED AT recal_90: the constant now reads 93.0, 110.64, 107.55.')
    note('  DEF_TOP — which is what recal_80 actually re-solved — is UNCHANGED at 107.55, and recal_90')
    note('  re-measured it deliberately: 107.5558 against the anchor, ONE card (Ben Wallace ’03) 0.0058')
    note('  above, which already lands ON 99 through the band. That is the same sub-hundredth rounding')
    note('  gap recal_84 measured and left, so it was left again. This pin MISSes on the OFF half only.')
    src('  and the multiplier', OVR, /d_score\(p\) \* 1\.1305/, '1.03 -> 1.1305')
    note('  THE CONSTANTS AND WHY: the multiplier is solved so Gary Payton ’96 returns to 86, his value')
    note('  at v73 — the baseline was snapshotted BEFORE any weight moved, so the re-solve absorbs this')
    note('  round only and not the four before it. DEF_TOP is re-derived by recal_67’s own doctrine as')
    note('  the measured maximum raw, so the summit lands ON 99. That repairs a drift worth naming: after')
    note('  r76/r81/r82 NOTHING in the pool reached 99 — the top was Ben Wallace ’04 at 98. r60’s')
    note('  REF_DRTG intercept underneath was not re-opened.')
    line("THE +-1 TEST: Gary Payton '96", 'DEF 86 -> 86, exact', 'the round’s scale-neutrality anchor holds', true)
    note('THE NINE-MAN TABLE (perdef / rimprot beside each delta):')
    note("  Gary Payton '96      perdef 94 rimprot 21   86 -> 86  (+0)")
    note("  Bruce Bowen '05      perdef 94 rimprot 56   85 -> 85  (+0)")
    note("  Jrue Holiday '21     perdef 94 rimprot 62   87 -> 92  (+5)")
    note("  Kawhi Leonard '17    perdef 93 rimprot 60   92 -> 95  (+3)")
    note("  Scottie Pippen '94   perdef 94 rimprot 74   83 -> 92  (+9)  [BIG BRANCH — see below]")
    note("  Marcus Smart '22     perdef 92 rimprot 30   81 -> 82  (+1)")
    note("  Tony Allen '15       perdef 93 rimprot 59   83 -> 87  (+4)")
    note("  Trae Young '22       perdef 34 rimprot 10   35 -> 34  (-1)")
    note("  Luka Dončić '23      perdef 64 rimprot 45   70 -> 73  (+3)")
    note('  The mechanism does what it claims: wings with real rim presence gain most (Jrue +5, Tony')
    note('  Allen +4, Kawhi +3) and the men with no rim contest gain least or lose (Payton rimprot 21,')
    note('  Bowen 56, Smart 30). PIPPEN ’94 IS ON THE BIG BRANCH — the round lists him as a perimeter')
    note('  receipt, but is_big routes him to 0.40/0.40, so item 1 never touched him; his +9 is the')
    note('  display re-solve alone.')
    line('RED LINE BREACHED — flagged, and shipped per his ruling', "Luka '23 DEF 70 -> 73, a RISE; the round says Trae and Luka must NOT rise", "Trae holds at -1; Luka does not — his rimprot is 45, so the new 0.13 term pays him", false)
    line('CONSEQUENCE THE ROUND DID NOT BUDGET FOR — for his eyes', 'the display re-solve moved ALL 4,608 bigs (mean 64.88 -> 71.12) and lifted the whole scale: 8,270 cards rose, 977 fell, mean +3.87', '"bigs unchanged entirely" cannot hold once the display pair moves', true)
    note('  WHY, plainly: the multiplier lifts every card BELOW the 93 knee, and branch membership is')
    note('  irrelevant there — only DEF_TOP touches the men above it. So restoring the perimeter class')
    note('  necessarily lifts the bigs too. And the anchor the round chose makes it worse: Payton’s')
    note('  rimprot is 21, near the floor, so he suffers the LARGEST deflation of anyone (-8 against a')
    note('  class mean of -3.1); calibrating the whole scale to restore him therefore overshoots the')
    note('  class it was meant to neutralise — perimeter mean 53.84 -> 55.69, not back to 53.84.')
    note('  SHIPPED ANYWAY, per his ruling, and named here rather than softened.')
    line('perimeter class, before / after item 1 / after the re-solve', '53.84 -> 50.74 -> 55.69 · 4,604 fell pre-re-solve, 3,662 of them restored or lifted after', 'the two-directional footprint the round did not ask for', true)
    line('summit pins re-verified', "GSW '17 OFF 99 · Pistons '04 DEF 99, both EXACT", 'recal_71 team-gauge law intact; r71 OFF_MIN/OFF_MID re-derived (104.36 / 123.27)', true)
    note("  Gobert '19 is a BIG and reads 92, not 99 — the round's item-2 condition \"Gobert stays 99\" was")
    note('  stale twice over: he was at 84 before this round, and NOTHING in the pool was at 99 at all.')
    note('  The operative condition was Payton +-1, and it holds exactly. Cards at DEF 99 now: 4.')
    line('discipline', 'parity green · mono 500/500 CLEAN (the pre-existing usage-economy pair is no longer sampled) · harness re-ratified (crashOff .30->.55, crashDef .62->.95, all nine in band) · suite 0/7 both bases · wall-vs-sieve spread 2.61, UNMOVED', 'full battery', true)
    note('  Wins gate against r_off 0.510 / r_def 0.496: now 0.513 / 0.492 — both flat. defenseVs reads')
    note('  ATTRS, not d_ovr, so the team engine never saw this round; the spread question stays exactly')
    note('  where it was for his ruling. One test pin re-anchored with its reason: Shaq ’00 D 81 -> 88.')
  },
  '88': () => {
    console.log(`${EOL}recal_88 — TWO-LEVEL ZONE DOMINANCE, DECLINED (his ruling, verbatim: "Dont do 87, only shift the weights a little torward 88")`)
    note('NUMBERING, so the ledger reads straight: the dispatcher had been calling this round "87" in')
    note('  its own messages, and receipt 87 was meanwhile spent on the wall-vs-sieve closure. The')
    note('  DECLINED two-level round is THIS receipt, 88; the weight shift his same sentence approved')
    note('  is receipt 89. Design-side called them "recal round 75" and "recal round 76". His words')
    note('  "Dont do 87 ... torward 88" are his own numbering, not ours, and are quoted unaltered.')
    note('WHAT WAS PROPOSED, in brief, so the decline is legible without the original round in hand:')
    note('  a SECOND dominance branch for the two-level scorer — z0 >= 90 AND z1 >= 85 AND z2 < 55 —')
    note('  paying up to 5.0 points, max()’d against the existing single-zone bonus so a man collected')
    note('  the better of the two. The target was the shape that towers in two ranges and is empty in')
    note('  the third, which the single-zone gate cannot see because its own second level disqualifies')
    note('  it (z0 must beat z1 + z2, and an 85 second zone makes that impossible).')
    line('DECLINED, not deferred', 'no branch built · no constant added · no version bump', 'his ruling, applied as a decision rather than left to go quiet', true)
    note('WHY IT WAS IN FRONT OF HIM AT ALL, and why that mattered: the round’s own safeguard against')
    note('  paying a thin, noisy second zone was the r70 ZONE CONFIDENCE shrink. Recal 78 REFUSED that')
    note('  mechanism on the merits, not on its constants — zone ratings are volume-aware BY')
    note('  CONSTRUCTION, so attempts are part of the measured quantity rather than a sample size, and')
    note('  shrinking by them rewards shot-diet avoidance (Korver ’15 rim 19 -> 28, receipt 78). So this')
    note('  round arrived asking for a 5-point bonus on a second zone WITHOUT the thin-sample guard it')
    note('  assumed it had. That is what was put to him, and he declined the branch.')
    // ---- the accepted consequence, measured on the shipped pool ----
    const shape = PLAYERS.filter((q) => q.attrs.rim >= 95 && q.attrs.mid >= 90 && q.attrs['3pt'] <= 40)
    note('THE ACCEPTED CONSEQUENCE — settled behaviour now, not an open hole. The shape he was shown')
    note('  (paint 98 / mid 94 / 3pt 31 on volume 98) is not hypothetical: it is MICHAEL JORDAN ’89,')
    note('  card for card. He collects NEITHER bonus, and after this ruling he never will:')
    for (const q of shape.slice(0, 3)) {
      const z = [q.attrs['3pt'], q.attrs.rim, q.attrs.mid].sort((x, y) => y - x)
      const single = Math.max(q.attrs['3pt'], q.attrs.rim) >= q.attrs.mid && ((z[0] > z[1] + z[2] && z[0] >= 91) || z[0] > 1.5 * (z[1] + z[2]))
      line(`  ${q.name}`, `rim ${q.attrs.rim} mid ${q.attrs.mid} 3pt ${q.attrs['3pt']} vol ${q.attrs.volume} · OFF ${q.o_ovr}`, single ? 'single-zone bonus' : 'NO bonus — too broad for the single-zone gate, too narrow for the third-zone weight', !single)
    }
    note('  THE ARITHMETIC OF THE HOLE, stated once so nobody re-derives it: the single-zone gate needs')
    note('  z0 > z1 + z2, and MJ ’89 is 98 vs 94 + 31 = 125, so he fails it by 27 points and cannot pass')
    note('  it at any volume. At the other end the third-zone weight pays 0.05 x 31 = 1.55 (recal_89’s')
    note('  new weight; it was 0.06 x 31 = 1.86 before), which is the whole of what his empty third')
    note('  level contributes. He is paid as a two-level scorer by the STANDING weights and by nothing')
    note('  else. That is the settled answer.')
    note('  AND IT COSTS HIM NOTHING VISIBLE: MJ ’89 reads OFF 96 either way. The hole the round wanted')
    note('  to fill was never costing the men it named a rating they could see — which is worth knowing')
    note('  before anyone re-opens it. If a two-level scorer should be paid MORE, that is a ruling with')
    note('  a named mechanism and a thin-sample guard that survives recal_78, not this branch again.')
  },
  '90': () => {
    console.log(`${EOL}recal_90 — THE OFF BAND ANCHOR, RE-DERIVED (his ruling, verbatim: "Fix the OFF band anchor"; completes the drift recorded at the end of receipt 89, and is the SIBLING of recal_84)`)
    const V90 = Number((OVR.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    const V90b = Number((RATINGS.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    line('PIPELINE_VERSION', `shipped at 80 · current ${V90} / ${V90b}`, 'both 80', V90 === 80 && V90b === 80)
    src('the re-derived anchor', OVR, /KNEE, OFF_TOP, DEF_TOP = 93\.0, 110\.64, 107\.55/, '106.36 -> 110.64, DEF untouched')
    src('the standing DEF diagnostic', OVR, /_dtops = \[\]/, 'DEF now prints its own top, as OFF has')
    note('THE READING TAKEN, named explicitly because recal_84’s had to be argued for. For OFF there is')
    note('  NO ambiguity, and that is the finding rather than a formality: the pipeline appends the')
    note('  band’s OWN ARGUMENT to _otops one line before calling it —')
    note('      _o = o_score(p) * 0.93 ;  _otops.append(_o) ;  band(_o, OFF_TOP)')
    note('  so the value the band CONSUMES is o_score AFTER the standard weighted path, AFTER the')
    note('  volume x efficiency interaction term, AFTER the zone-dominance bonus and AFTER the 0.93')
    note('  display multiplier. Nothing has to be reconstructed, unlike OVR where the tax and breadth')
    note('  sat between the blend and the band and the reading had to be chosen.')
    note('  THE ONE OTHER DEFENSIBLE READING, considered and REJECTED: derive from the raw BEFORE the')
    note('  dominance bonus, so a weapon could be paid past the ceiling. Rejected for r84’s reason —')
    note('  the band’s entire function is to map ITS OWN INPUT onto 99, so an anchor derived from any')
    note('  other quantity cannot make the top card land on 99, which is all the constant is for.')
    line('THE DERIVATION, from the pool rather than typed', 'max over 10,000 cards = 110.6400 — Giannis Antetokounmpo ’25', '106.36 -> 110.64', true)
    note('  WHY IT DRIFTED: recal_89’s weight shift toward load and creation lifted the very top of the')
    note('  offensive raw, 110.08 -> 110.64, and carried a SECOND card past the old anchor. Giannis ’25')
    note('  (110.6400) and ’24 (106.4581) both clamped and tied at 99 — the same compression recal_84')
    note('  removed from OVR, reappearing on OFF. The run now prints "0 cards above the anchor".')
    line('cards above the anchor', '2 -> 0', 'nothing is clamped by the ceiling any more', true)
    // ---- the 99 tier ----
    const at99 = PLAYERS.filter((q) => q.o_ovr === 99)
    note('THE 99 TIER, before -> after:')
    note('  BEFORE (n=6): Giannis ’24, Giannis ’25, Jokic ’25, Jokic ’26, Shaq ’00, Shaq ’02')
    note(`  AFTER  (n=${at99.length}): ${at99.map((q) => q.name).join(', ')}`)
    line('EXACTLY ONE LANDS ON THE ANCHOR BY CONSTRUCTION', `${at99.length} card at OFF 99`, 'Giannis ’25, whose raw IS the anchor', at99.length === 1 && at99[0].name === "Giannis Antetokounmpo '25")
    note('  This is the CLEANER outcome of the two sibling rounds, and the difference is worth naming:')
    note('  recal_84 left TWO cards tied at the OVR summit and reported the tie as honest rather than')
    note('  forcing them apart. Here the arithmetic separates them on its own — Giannis ’25 maps to')
    note('  exactly 99.000 because his raw IS the top, and Giannis ’24 at 106.4581 maps to 97.58 and')
    note('  rounds to 98. Nothing was forced; the six-way tie simply stopped existing.')
    for (const [n, was] of [["Giannis Antetokounmpo '25", 99], ["Giannis Antetokounmpo '24", 99]] as const)
      line(`  ${n}`, `OFF ${was} -> ${g(n).o_ovr}`, 'the two that were tied at the ceiling', true)
    // ---- the multiplier verdict ----
    src('the 0.93 display multiplier, untouched', OVR, /_o = o_score\(p\) \* 0\.93/, 'not re-solved')
    note('THE MULTIPLIER VERDICT, addressed head on because re-deriving a band top could have forced it:')
    note('  THE TWO CONSTANTS DO NOT CONFLICT, AND THEY CANNOT, because they own DISJOINT REGIONS of')
    note('  the scale. band() is the IDENTITY below the knee — a raw of 74 is a card of 74 — so 0.93')
    note('  alone decides where the ~98% of the pool below 93 sits, which is exactly r34’s stated job')
    note('  for it ("the new weight sum is ~15% larger, so the multiplier is re-normalised to keep the')
    note('  scale where it was"). OFF_TOP alone decides how the thin tail above 93 is stretched onto')
    note('  93-99. Changing the top cannot move a card the multiplier governs, and MEASURED: not one')
    note('  card below OFF 94 moved by a single point (the lowest mover in the whole footprint is')
    note('  Barkley ’91 at 94). So the multiplier still holds its own doctrine and is NOT re-solved.')
    note('  DOES r89’s AMENDMENT TO r34’s LOCKED DIAL STATE SURVIVE? YES, and untouched. r89 amended')
    note('  that pin on the grounds that the total weight was unchanged, so the renormalised 0.93 was')
    note('  still correct. recal_90 changes NO WEIGHT AT ALL — it changes one band anchor — so it')
    note('  cannot disturb that reasoning. Checked rather than assumed, because it was the obvious')
    note('  place for two constants to start silently disagreeing.')
    // ---- DEF ----
    line('THE DEF ANCHOR, checked while in the constant', 'measured 107.5558 vs anchor 107.55 — ONE card above, by 0.0058', 'LEFT, deliberately: the same rounding gap r84 measured', true)
    note('  The single card is Ben Wallace ’03, and he already lands ON 99 through the band, so the')
    note('  anchor is doing its job. THIS IS NOT THE SAME DRIFT: OFF had TWO cards tied at the ceiling')
    note('  with a 4.28-point gap behind the leader; DEF has one card six thousandths above a constant')
    note('  that still maps it to 99. One sanctioned outlier that lands on 99 is what a ceiling is for,')
    note('  which is r84’s own sentence, and recal_86 moving 1,823 perdefs did not change that.')
    note('  MADE PERMANENT: DEF now carries the same standing diagnostic OFF has. recal_84 had to')
    note('  hand-audit this constant because nothing printed it, and this is the SECOND anchor to drift')
    note('  unseen — so the run now prints "DEF raw top X (band anchor Y) — N cards above the anchor"')
    note('  every time. A one-line diagnostic is cheaper than a third hand audit.')
    // ---- ordering + footprint ----
    line('ORDERING in the top-20 union', 'NONE — zero pairs inverted', 'checked pairwise across the union, not row by row', true)
    line('WHAT THE CEILING CHANGE DID TO THE TOP', 'SEPARATED it', 'a ceiling move can only compress or separate — this one separated', true)
    note('  99: 6 -> 1 · 98: 6 -> 1 · 97: 10 -> 7 · 96: 11 -> 14. The six-way tie at the ceiling became')
    note('  1 + 1 + 4 (Giannis ’25 at 99, Giannis ’24 at 98, then Jokic ’25/’26 and Shaq ’00/’02 at 97).')
    note('  HONEST QUALIFIER: four of the six still tie, one step lower. The tie AT THE CEILING is gone,')
    note('  which is what the round was for; a four-way tie at 97 is ordinary board density, not a clamp.')
    line('FOOTPRINT — both directions, as standing practice requires', '48 OFF cards moved · 0 rose · 48 fell · 9,952 unchanged · mean -1.15 · nothing moved more than 2', 'tiny and confined above the knee, as predicted', true)
    note('  CONFINEMENT VERIFIED, not assumed: the lowest-rated mover in the entire pool is Charles')
    note('  Barkley ’91 at OFF 94. Not one card below 94 moved by a point, which is the band being the')
    note('  identity below its knee. 41 cards moved -1 and 7 moved -2. Every mover is an all-time')
    note('  offensive season: Giannis ’18-’25, Shaq ’97-’05, LeBron ’08-’13, Jokic ’21-’26, Durant ’14/’16/’19,')
    note('  Jordan ’88/’90/’91, Curry ’15/’17, SGA ’24-’26, Embiid ’23/’24, Malone ’92/’98, Barkley ’91,')
    note('  Robinson ’95, Kobe ’07, Harden ’15, Isaiah Thomas ’17.')
    note('  DIRECTION IS ONE-WAY BY CONSTRUCTION, and that is not a bias: widening the top of a band')
    note('  can only ever lower what sits inside it. There is no upward half to report, and saying so')
    note('  is more honest than presenting an empty "rose" column as balance.')
    note('  OVR followed on 37 cards, all down, all elite. OFF pool mean 55.6066 -> 55.6011 (-0.0055).')
    line('discipline', 'parity green · mono68 500/500 with 0 violations · 93 tests green · NO test pin needed re-anchoring · r71 gauge constants unchanged, summit pins EXACT (GSW ’17 OFF 99 · Pistons ’04 DEF 99) · suite 0/7 both bases', 'full battery', true)
    note('  MONO68: clean again, and again this is NOT a fix. The usage-economy violation receipt 86')
    note('  carries (Nazr Mohammed ’05 -> Kemp ’91) did not recur because the sampler draws on OVR and')
    note('  the OVRs it draws from barely moved. A clean 500/500 is the ABSENCE OF A DRAW, not the')
    note('  presence of a guarantee. The flaw is unfixed and stays named until a round fixes it.')
    note('  THE HARNESS NEEDED NO RE-RATIFICATION — all nine tactics in band on untouched constants,')
    note('  and hunt held at 3.60. THIS ANSWERS THE QUESTION ITS LAST THREE MOVES RAISED: hunt moved at')
    note('  r85, r86 and r89, reversing direction once, and the honest reading was that this is THE')
    note('  POOL rather than the tax. recal_90 is the control that shows it. It moved 48 cards, all')
    note('  above OFF 94, so the ovr>=55 harness pool is effectively identical — and hunt did not move')
    note('  a thousandth. A tax that wanders only when the pool underneath it wanders is tracking the')
    note('  pool, which is what the r59 law asks it to do.')
    line('WINS GATE', 'r_off 0.501 -> 0.501 · r_def 0.510 -> 0.510', 'unchanged to three decimals; the 0.6 gate still FAILS', true)
    note('  Expected and confirmed: the team engine reads ATTRS, and this round moved a DISPLAY band.')
    note('  No attribute changed, so no route existed by which the gate could move. Not tuned toward.')
  },
  '89': () => {
    console.log(`${EOL}recal_89 — THE o_score WEIGHT SHIFT, A TINY BIT TOWARD LOAD (design-side round "76"; his ruling: "Dont do 87, only shift the weights a little torward 88")`)
    const V89 = Number((OVR.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    const V89b = Number((RATINGS.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    line('PIPELINE_VERSION', `shipped at 79 · current ${V89} / ${V89b}`, 'both 79', V89 === 79 && V89b === 79)
    src('the shifted weights', OVR, /std = \(0\.22\*z\[0\] \+ 0\.08\*z\[1\] \+ 0\.05\*z\[2\] \+ 0\.11\*a\['efficiency'\] \+ 0\.26\*a\['volume'\] \+ 0\.19\*a\['playvol'\]/, 'zones 0.25/0.09/0.06 -> 0.22/0.08/0.05, eff 0.10 -> 0.11, vol 0.24 -> 0.26, playvol 0.17 -> 0.19')
    src('everything else untouched', OVR, /\+ 0\.10\*a\['ballsec'\] \+ 0\.11\*\(a\['fouldraw'\]\*a\['ft'\]\/100\) \+ 0\.06\*a\['orb'\]/, 'ballsec .10, fouldrawft .11, orb .06')
    src('the interaction term untouched', OVR, /\+ 0\.08\*\(max\(a\['volume'\],50\)\*a\['efficiency'\]\/100\)\)/, '0.08 x (max(volume,50) x eff / 100)')
    note('PREMISE CHECK: CLEAN — and this is the first round in ten where it is. All ELEVEN constants')
    note('  the round quotes are the ones we actually ship: zones 0.25/0.09/0.06, efficiency 0.10,')
    note('  volume 0.24, playvol 0.17, ballsec 0.10, fouldrawft 0.11, orb 0.06, interaction 0.08,')
    note('  display multiplier 0.93, KNEE 93 / OFF_TOP 106.36. Nothing had to be corrected before')
    note('  applying it. Recorded because the last nine rounds all needed a correction first.')
    note('ONE SENTENCE OF THE ROUND IS NOW FALSE, and it is not left to stand: it says the dominance')
    note('  bonuses "(single-zone, and the r75 two-level branch)" read the zone RATINGS and so are')
    note('  unaffected. The r75 TWO-LEVEL BRANCH DOES NOT EXIST AND NEVER WILL — Tomer declined it in')
    note('  the same sentence that approved this shift (receipt 88). The claim is correct for the ONE')
    note('  bonus that exists: the single-zone gate reads z[] ratings, not these weights, and is')
    note('  verified untouched above.')
    // ---- scale neutrality: a CLAIM, measured ----
    const mean = (xs: number[]) => xs.reduce((a, b) => a + b, 0) / xs.length
    const offs = PLAYERS.map((q) => q.o_ovr)
    line('SCALE NEUTRALITY — OFF pool mean', `55.855 -> ${mean(offs).toFixed(3)} · delta -0.248`, 'the round allows +-0.3', Math.abs(mean(offs) - 55.855) <= 0.3)
    note('  IT HOLDS, BUT NOT "BY CONSTRUCTION" AS THE ROUND CLAIMS. The round argues the mean cannot')
    note('  move because 0.05 goes out and 0.05 comes in on a shared 1-99 scale. Sharing a SCALE is not')
    note('  sharing a DISTRIBUTION: z[0] is a MAX of three zones and sits far above the pool mean, while')
    note('  volume and playvol sit near or below it, so moving weight from the one to the others lowers')
    note('  the average card. It moved -0.248 — inside the tolerance, but 83% of the whole budget. Had')
    note('  the shift been twice as large, as "a tiny bit" might easily have meant, it would have BROKEN')
    note('  the round’s own gate. Measured, not assumed, and the reason it nearly failed is named.')
    note(`  pool sd 15.407 -> 15.450 — the distribution widens very slightly rather than compressing.`)
    // ---- the nine named cards ----
    note('THE NINE NAMED CARDS, OFF before -> after:')
    const NAMED: [string, number][] = [["Stephen Curry '16", 95], ["Michael Jordan '89", 96], ["Kawhi Leonard '17", 94], ["LeBron James '13", 97], ["Shaquille O'Neal '00", 98], ["Kyle Korver '15", 59], ["DeMar DeRozan '17", 86], ["Chauncey Billups '06", 85], ["Jaylen Brown '26", 84]]
    for (const [n, was] of NAMED) {
      const q = g(n)
      line(`  ${n}`, `OFF ${was} -> ${q.o_ovr}`, 'the round predicts roughly flat stars, easing specialists', Math.abs(q.o_ovr - was) <= 2)
    }
    note('  READ IT: the four complete stars are EXACTLY flat (Curry 95, MJ 96, Kawhi 94, LeBron 97),')
    note('  which is what the round predicted. The specialists ease as predicted but by ONE point, not')
    note('  more: Korver 59 -> 58, Kerr ’96 63 -> 62, Reggie ’97 88 -> 88 (flat). Shaq ’00 98 -> 99 and')
    note('  Billups ’06 85 -> 86 are the load-and-creation gains. Brown ’26 84 -> 83.')
    note('THE ROUND’S NAMED WINNERS DID NOT WIN, and this is the honest half of the report: it expected')
    note('  to "gain 1-2 high-volume creators (SGA, Jokic, Luka, Harden, Westbrook)". Measured, ALL FIVE')
    note('  are flat: SGA ’25 97 -> 97, Jokic ’25 99 -> 99, Luka ’23 94 -> 94, Harden ’19 95 -> 95,')
    note('  Westbrook ’17 91 -> 91. They are already at or against the top of the band, where a 0.05')
    note('  weight nudge cannot show through the rounding. The gain is real in the raw and invisible on')
    note('  the card. NOT tuned to make the prediction come true.')
    // ---- the top board ----
    note('THE TOP-20 OFF BOARD — the falsifiable part, and it is reported as ORDERING, not as a delta:')
    note('  BEFORE  99 (n=4): Giannis ’24, Giannis ’25, Jokic ’25, Jokic ’26')
    note('          98 (n=4): LeBron ’10, Shaq ’00, Shaq ’01, Shaq ’02')
    note('          97 (n=13): Giannis ’20/’22/’23, LeBron ’09/’12/’13, Jokic ’21/’22/’24, SGA ’25/’26, Shaq ’03/’99')
    note('  AFTER   99 (n=6): + Shaq ’00, Shaq ’02')
    note('          98 (n=6): Giannis ’20, LeBron ’09/’10/’12, Jokic ’22, Shaq ’01')
    note('          97 (n=10): Giannis ’19/’22/’23, LeBron ’13, Jokic ’21/’24, SGA ’25/’26, Shaq ’03/’99')
    line('ORDERING CHANGES in the top-20 union', 'NONE — zero pairs inverted', 'every pair keeps its relative order; only tie-groups re-form', true)
    note('  This was checked pairwise across the UNION of both top-20s rather than by comparing row')
    note('  numbers, because comparing rows makes tie-shuffling look like churn: a naive row-by-row')
    note('  diff shows 16 of 20 rows "changed" and every one of them is a tie re-sorting. Not one man')
    note('  passed another. Giannis ’19 enters the top 20 and Shaq ’03 leaves it, both on ties.')
    line('FOOTPRINT — both directions, as standing practice requires', '4,600 o_ovrs moved · 1,198 rose · 3,402 fell · mean -0.540 · 5,400 unchanged · OVR 2,537 moved (674 up, 1,863 down)', 'a redistribution that is NOT symmetric', true)
    note('  THE ASYMMETRY IS THE STORY AND THE ROUND DOES NOT MENTION IT: three cards fall for every one')
    note('  that rises. Nobody moves more than 2 in either direction, so this is not a re-rating — it is')
    note('  the whole middle of the pool sliding down half a point while a minority of load-carriers')
    note('  hold or gain. Largest falls (-2): AJ Griffin ’23, Aaron Nesmith ’24, Ace Bailey ’26, Allen')
    note('  Crabbe ’18, Amir Coffey ’24/’25 — all low-volume shooters, exactly the class the round')
    note('  intended to ease. Largest rises (+2): Tyus Edney ’97, Tiny Archibald ’80, Westbrook ’10,')
    note('  Ramon Sessions ’09, Micheal Williams ’92, Kenny Anderson ’97 — all high-playvol guards.')
    note('  The intent is delivered. The 3:1 ratio is the part to know about, and it is why the pool')
    note('  mean spent 83% of its tolerance.')
    note('DRIFT FOUND WHILE IN THERE, NAMED NOT FIXED — the OFF band anchor. recal_84 audited this')
    note('  constant and LEFT it deliberately, on the reasoning that "one card past the anchor IS the')
    note('  sanctioned case; four is not". Measured now: OFF raw top 110.08 -> 110.64 and the count')
    note('  above OFF_TOP = 106.36 went ONE -> TWO. Both are Giannis: ’25 at 110.640 (far above) and')
    note('  ’24 at 106.458 (barely). Both clamp to 99, so they now TIE at the ceiling — which is the')
    note('  same compression recal_84 removed from OVR, reappearing on OFF. NOT re-derived here: this')
    note('  round is a weight shift with a stated scope, r84’s decision to leave OFF alone is on the')
    note('  record, and re-deriving a band anchor to relieve a tie is a round of its own. Flagged so')
    note('  the second card is a known fact rather than something discovered at four.')
    line('discipline', 'parity green · mono68 500/500 with 0 violations · 93 tests green · r71 gauge constants unchanged, summit pins EXACT (GSW ’17 OFF 99 · Pistons ’04 DEF 99) · suite 0/7 both bases', 'full battery', true)
    note('  MONO68 IS CLEAN THIS ROUND, and that is worth stating precisely: the usage-economy violation')
    note('  receipt 86 carries (Nazr Mohammed ’05 -> Kemp ’91) did NOT recur, because the sampler draws')
    note('  on OVR and 2,537 OVRs moved, so it drew different fives. The flaw is NOT fixed. A clean')
    note('  500/500 here is the absence of a draw, not the presence of a guarantee.')
    note('  THE DEVIATION TAX LAW: hunt again, and again reversing — its ORACLE fell to +0.46 (r86 had')
    note('  fixed a BLIND failure), so the tax came back DOWN 3.72 -> 3.60 and reads random -0.38 /')
    note('  oracle +0.51. Third round running that hunt is the only band to move; it is the most')
    note('  pool-sensitive tax we have. crash def glass reads -0.31, off the edge it sat exactly on at')
    note('  r86, and was again not touched.')
    line('WINS GATE — r_off is what this round was expected to move', 'r_off 0.501 -> 0.501 · r_def 0.510 -> 0.510', 'reported honestly: it did NOT move, to three decimals', true)
    note('  Both sides are unchanged and both still fail the 0.6 gate. The shift moved 4,600 cards and')
    note('  did not move the 26-team correlation by a thousandth. That is real information: the wins')
    note('  gate is not sensitive to o_score WEIGHTS at all, which narrows where a future fix can live.')
    note('  Not tuned toward, and not dressed up as progress.')
  },
  '87': () => {
    console.log(`${EOL}recal_87 — THE 60/40 DEFENSIVE SPREAD IS RETIRED, ACCEPTED AS IT STANDS (his ruling, verbatim: "Ship the wall-vs-sieve spread as is")`)
    const DEF = io('tests/defense.test.ts')
    line('PIPELINE_VERSION', 'untouched · current 78', 'a ruling on an existing figure — zero cards move, no bump', /PIPELINE_VERSION = 78/.test(RATINGS) && /PIPELINE_VERSION = 78/.test(OVR))
    src('the pending marker is CLOSED, not deleted', DEF, /PENDING TOMER IS RETIRED HERE/, 'the history stays in the file, annotated')
    src('his ruling is quoted at the assertion', DEF, /"Ship the wall-vs-sieve spread as is"/, 'verbatim, next to the number it settles')
    src('the shape is a FLOOR', DEF, /expect\(dS - dW\)\.toBeGreaterThanOrEqual\(2\.0\)/, '2.4 -> 2.0, and not a pin')
    note('WHY A FLOOR AND NOT A PIN OR A BAND — stated so the choice is auditable:')
    note('  this number has moved with almost every card round that touched perdef, and it moves')
    note('  BECAUSE perdef moves, which is the pipeline working rather than failing. An exact pin')
    note('  would make every future round trip over a figure he has just said he does not mind. A')
    note('  band would need an upper edge nobody has ruled on. A floor catches only the thing he did')
    note('  NOT accept — a collapse toward zero, where defence stops being a property of the five.')
    note('  THE FLOOR IS 2.0, chosen for a reason rather than for headroom: at 2.0 the sim pays 0.40')
    note('  points a game, so a break means the accepted half-point has eroded by more than a fifth.')
    note('  The CEILING of 11 is kept as a runaway guard. It was never part of the pending item.')
    note('THE HISTORY, since this decision outlived four rounds and the record should carry it whole:')
    note('  ORIGINAL LAW  the 60/40 doctrine wanted a wall-vs-sieve spread of 8-10 DRtg.')
    note('  r36           9.59 -> 7.85 (height entered perdef).')
    note('  r60           7.85 -> ~5   (the lone-hunted-man penalty moved out of the standalone')
    note('                rating and into the ASSIGNMENT lever, where the optimal board pays zero).')
    note('  r76 BROKE IT  8-10 -> 3.53. His ruling "Remove team Def rating from per def" cost the')
    note('                elite class the team credit perdef was double-paying (Kawhi ’17 D 99->92,')
    note('                Shaq ’00 89->81, Gobert ’19 87->84). WALL is selected DYNAMICALLY at')
    note('                perdef >= 85, so its membership moved with them: the round changed WHO the')
    note('                elite defenders are. r76 also established DRTG_COEF is NOT the lever, and')
    note('                that finding is not re-opened here.')
    note('  r81 MOVED IT  3.53 -> 2.61, closing rimprot’s DBPM door — the second half of the same')
    note('                team-defence ruling, so the same mechanism a second time.')
    note('  r85 LEFT IT   the OVR round touched no attribute; the spread did not move.')
    note('  r86 NEW CAUSE 2.61 -> 2.46, and for a DIFFERENT reason than r76/r81. The absolute tracked')
    note('                perdef branch moved the FLOOR under everyone else instead of the elite: the')
    note('                WALL pool (perdef >= 85) did not change by a single card, 351 -> 351,')
    note('                because the elite are pinned by their own absolute DFG floors, while the')
    note('                sieve tier (perdef <= 35) went 525 -> 346 and population sd 15.33 -> 14.90.')
    note('                The wall stood still and the sieve came up under it. THAT REVERSES r76’s')
    note('                recorded finding that the population had not compressed. He ruled with the')
    note('                reversal on the table, which is why this closes rather than defers again.')
    // ---- what is now unprotected, measured ----
    const oppL = (JSON.parse(io('src/data/opponents.json')) as { players: Player87[] }[])[14].players
    type Player87 = (typeof PLAYERS)[number]
    const seen87 = new Set<string>()
    const dis = (xs: Player87[]) => xs.filter((q) => !seen87.has(q.player) && seen87.add(q.player))
    const WALL87 = dis([...PLAYERS].filter((q) => q.attrs.perdef >= 85 && q.attrs.ts_raw < 0.53).sort((a, b) => b.attrs.perdef - a.attrs.perdef)).slice(0, 5)
    seen87.clear()
    const SIEVE87 = dis([...PLAYERS].filter((q) => q.attrs.ts_raw >= 0.58 && q.attrs.usg_raw >= 22).sort((a, b) => a.attrs.perdef - b.attrs.perdef)).slice(0, 5)
    const dW87 = defenseVs68(WALL87, oppL).drtg
    const dS87 = defenseVs68(SIEVE87, oppL).drtg
    const sp87 = dS87 - dW87
    line('the accepted spread, measured now', sp87.toFixed(2), '2.46, at or above the 2.0 floor', sp87 >= 2.0)
    line('WHAT IT IS WORTH IN THE GAME', `${(K_MATCH * sp87).toFixed(2)} margin points per game`, 'the defense term is K_MATCH x (B.drtg - A.drtg), K_MATCH = 0.2', true)
    note('WHAT IS NOW UNPROTECTED — the plain statement the ruling deserves. The 60/40 law existed to')
    note('  guarantee a FELT difference between a wall and a sieve. At 2.46 that difference is:')
    note('    accepted today   0.49 points per game  ->  game 52.0%, best-of-7 54.3%')
    note('    old law 8-10     1.60-2.00 points/game ->  game 56.4-57.9%, series 63.7-66.9%')
    note('  In words: fielding the five best perimeter defenders in the pool instead of the five')
    note('  WORST, with the SAME offence and against the SAME opponent, is worth HALF A POINT a game.')
    note('  Under the law this ruling retires it was worth two. A five of walls beats a five of sieves')
    note('  in a series 54.3% of the time, where a coin flip is 50.0%. DEFENCE IS NOW A PREFERENCE,')
    note('  NOT AN EDGE. He has accepted that, and it is written here so it is never re-discovered as')
    note('  a bug. If the draft should reward a defensive build more than this, that is a NEW round')
    note('  with a named mechanism — not a re-derivation of DRTG_COEF, which r76 ruled out.')
    note(`  WALL : ${WALL87.map((q) => `${q.name} ${q.attrs.perdef}`).join(' · ')}`)
    note(`  SIEVE: ${SIEVE87.map((q) => `${q.name} ${q.attrs.perdef}`).join(' · ')}`)
    note('STILL OPEN, and NOT closed by this ruling — two sibling bands in the same file scale with')
    note('  the same DRTG_COEF and he ruled on one of them: the recal_12 anchor-isolation swing (3.62')
    note('  against a band of 4) and the passqual-removal margin ceiling. Their status is now CLEARER,')
    note('  because the coefficient underneath them is fixed by an ACCEPTED figure rather than a')
    note('  pending one — but they are still his to rule on. Annotated in place, assertions untouched.')
    line('discipline', 'tsc clean · 93 tests green · no pipeline run, no version bump, zero cards moved', 'a bookkeeping round does exactly nothing to the data', true)
  },
  '86': () => {
    console.log(`${EOL}recal_86 — THE TRACKED PERDEF BRANCH BECOMES ABSOLUTE AND RE-CENTRED (design-side round "74"; their numbering collides with ours, so theirs 74 = ours 86)`)
    const V86 = Number((RATINGS.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    const V86b = Number((OVR.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    line('PIPELINE_VERSION', `shipped at 78 · current ${V86} / ${V86b}`, 'both 78', V86 === 78 && V86b === 78)
    src('the absolute line', RATINGS, /return min\(84\.0, max\(25\.0, 58\.0 - 5\.0 \* \(100\.0 \* diff\)\)\)/, 'clamp(58 - 5.0 x diff_6plus, 25, 84)')
    src('the percentile is gone from the branch', RATINGS, /d_card = _abs_perdef\(dv\)/, '1 - Pperim(dv) retired')
    src('the 0.30 corroboration survives, mapped absolutely', RATINGS, /d_card = \(1 - ALLSHOT_W\) \* d_card \+ ALLSHOT_W \* _abs_perdef\(dv_all\)/, 'his "all shots carry weight" ruling intact')
    src('the two weights are untouched', RATINGS, /wm = 0\.70 \* _targeting_weight\(r\['name'\]\) \* _sample_weight\(r\['name'\]\)/, 'targeting AND sample, as today')
    note('PREMISE CHECK — THE ROUND QUOTES A BRANCH WE DO NOT HAVE. Three discrepancies, all material:')
    note('  1. It writes tw = 0.70 * min(1, att/350). Ours is wm = 0.70 * targeting * sample — TWO')
    note('     weights. Its own prose says "blend by sample confidence AS TODAY", and today includes')
    note('     the targeting weight (a hunted man leans back on the composite). Prose kept, formula not.')
    note('  2. It says the defect card "is at FULL tracking weight, so perdef is the tracked value')
    note('     alone". IMPOSSIBLE HERE: wm caps at 0.70, so 30% of the reading is ALWAYS the composite.')
    note('  3. Its replacement reads diff_6plus ALONE, silently deleting HIS OWN RULING that all shots')
    note('     carry weight (ALLSHOT_W = 0.30). An owner ruling outranks a design round, so the')
    note('     corroboration was PRESERVED and mapped through the same absolute line. The verbatim')
    note('     single-series variant was measured alongside: tracked-term mean 54.15 vs the shipped')
    note('     55.01. One line to flip if he rules the other way; not flipped on a design round.')
    note('  The floor ladder it calls "(r63)" is OURS and correct as a ladder — but it is recal_16,')
    note('  re-keyed to the 6ft+ series by recal_20/recal_55 and verified by recal_65. Only the number.')
    note('THE UNNAMED DEFECT CARD (drep 0, DBPM 0.3, 6-11, diff +0.8%, 398 shots, perdef 64): NOT')
    note('  REPRODUCIBLE. Every card in our tree on that diff/attempts signature is listed by')
    note('  scripts/perdef86.py; the only two at 6-11 are Cody Zeller ’15 (perdef 54) and Andre')
    note('  Drummond ’24 (perdef 45). Nothing reads 64. His 64 is a reading from a build BEFORE')
    note('  recal_76 (teamd out of perdef), recal_80 (DEF display re-solved to 1.1305 / DEF_TOP 107.55)')
    note('  and recal_82 (graded rimprot entry). The composite this branch blends against is not the')
    note('  one the round’s author measured, and the receipt says so rather than inventing agreement.')
    note('  THE SHARPER FINDING: +0.8% is not one man’s defect, it is THE POOL MEAN. The tracked')
    note('  population’s mean shrunk 6ft+ diff is +0.80pp exactly. So the round’s two predictions are')
    note('  mutually inconsistent: it wants this card at ~54 (58 - 5*0.8) AND the population near 58,')
    note('  but this card IS the population. On its own line the pool must land at 54, and it does.')
    line('tracked-term mean (card space)', '55.01', 'the round says "near 58" — it is 55.0 and CANNOT be 58', true)
    note('  ROOT CAUSE, measured not guessed: the line puts the population wherever its mean diff puts')
    note('  it. 58 - 5.0 * (+0.80) = 54.0 on the 6ft+ series alone; the 0.30 Overall corroboration')
    note('  (mean diff only +0.17pp) lifts it to 55.01. "Near 58" assumed the tracked pool averages a')
    note('  ZERO diff. The DERIVED 6ft+ series does not and cannot: subtracting the rim slice out of')
    note('  Overall leaves the shots defenders are worst at, so the series skews positive by')
    note('  construction. NOT TUNED TOWARD 58 — moving the 58 constant to hit a population mean would')
    note('  re-introduce exactly the pool-centring this round exists to remove.')
    line('CARD mean over tracked cards (n=2,972)', '53.24 -> 54.76', 'moves toward the line, damped by wm <= 0.70 and the voted blend', true)
    note('  distribution p10/p50/p90: 36/48/76 -> 39/50/76. The top does not move — it is pinned by the')
    note('  absolute DFG floors — and the bottom comes up, which is the re-centring doing its job.')
    line('THE RED LINE — floor-tier cards that moved DOWN', '0 of 849', '0, or STOP and report', true)
    note('  849 cards carry a binding r16/r20/r55 absolute floor. Not one fell. The round predicted')
    note('  "genuinely elite tracked defenders unchanged (their floors were already binding)" and that')
    note('  is confirmed: Afflalo ’16 (diff -2.00%) 70 -> 70 and Middleton ’15 (-4.98%) 76 -> 76 both')
    note('  sit ON their floors, which outrank the line by 1 to 0.5 points exactly as designed.')
    note('THE FIVE-DEFENDER TABLE, spanning diff +2% to -5% (diff · att · targeting · sample · tw):')
    note('  Svi Mykhailiuk ’21   +2.00%  319  0.79 0.91 tw 0.503   perdef 38 -> 42   (line 48.0)')
    note('  Malik Beasley ’22   +0.80%  400  0.62 1.00 tw 0.433   perdef 39 -> 41   (line 54.0)')
    note('  Gerald Henderson ’17 -0.50%  408  0.76 1.00 tw 0.529   perdef 43 -> 47   (line 60.5)')
    note('  Arron Afflalo ’16   -2.00%  523  0.52 1.00 tw 0.363   perdef 70 -> 70   (line 68.0, FLOOR 70 binds)')
    note('  Khris Middleton ’15 -4.98%  547  0.38 1.00 tw 0.266   perdef 76 -> 76   (line 82.9, FLOOR 76 binds)')
    note('  Read the tw column: the heavily-targeted men (Afflalo 0.52, Middleton 0.38) are exactly the')
    note('  ones the round’s single-factor tw would have trusted at 1.00. Keeping the targeting weight')
    note('  is why the elite end moved 0 instead of being dragged by volume.')
    line('FOOTPRINT — both directions, as standing practice requires', '1,823 perdefs moved · 1,756 rose · 67 fell · mean +2.51 · 8,177 unchanged', 'a re-centring, not a compression', true)
    note('  THE ROUND PREDICTED "mid-pack tracked defenders compress toward the 50s" AND NAMED NO')
    note('  RISERS. It is overwhelmingly a RISE: 26 men up for every 1 down. The mechanism is the')
    note('  re-centring itself — the old map sent a median tracked defender to card ~50.5 (0.17 +')
    note('  0.67*0.5), the new one sends a zero-diff defender to 58. The whole middle moves up ~7 in')
    note('  the tracked term before the weights damp it.')
    note('  BIGGEST RISERS, none of them mentioned by the round: Sean Kilpatrick ’17 32 -> 40 (+8),')
    note('  Shelvin Mack ’19 35 -> 42, Sergio Rodríguez ’17 32 -> 39, Ramon Sessions ’16 37 -> 44,')
    note('  P.J. Hairston ’16 36 -> 43, Mike Miller ’14 33 -> 40 (all +7). Every one is a low-minute,')
    note('  thin-sample guard who used to be read at the BOTTOM of a percentile and is now read on a')
    note('  line where his own modest diff is worth what it is worth. Whether a +8 for Sean Kilpatrick')
    note('  is right is a JUDGEMENT the round did not make; it is named here so it is his to make.')
    note('  BIGGEST FALLS are tiny by comparison, all -3: Aaron Nesmith ’26 54 -> 51, David Roddy ’23')
    note('  56 -> 53, Dennis Smith Jr. ’23 60 -> 57, Kris Murray ’26 60 -> 57, Spencer Jones ’26 56 -> 53.')
    line('pre-2014 path untouched', '66 cards with peak season < 2014 moved, none earlier than 2012', 'the branch cannot fire pre-2014 (tw = 0, no tracking rows)', true)
    note('  ALL 66 are peak season 2012 (9) or 2013 (57) — the SEASON-SMOOTHING window reaching forward')
    note('  into a tracked neighbour, a mechanism that predates this round. Nothing before 2012 moved by')
    note('  a single point. The branch itself is gated on a tracking row existing, and none do pre-2014.')
    note('THE FROZEN CONTROLS THE ROUND ASKED FOR — BOTH FAIL, and one of them could never have passed:')
    note('  Trae Young: NOT frozen. ’19 33->35, ’20 33->35, ’21 34->38, ’22 34->37, ’23 34->37, ’24 33->36,')
    note('    ’25 33->36. Receipt 55 has carried this same man as "THE FAILED FREEZE" since r55 — the')
    note('    round is asking us to re-confirm a freeze our own ledger records as broken. It cannot')
    note('    hold: an absolute line keyed on the diff MUST move a man whose diff is positive, and his')
    note('    is. Naming him as a control while re-centring the scale he sits on are incompatible asks.')
    note('  Luka: NOT frozen either. ’19 44->47, ’20 45->47, ’21 49->50, ’22 52->53, ’24 53->54, ’25 51->52.')
    note('    Same mechanism, smaller because his diffs are smaller. ’23 alone is genuinely unchanged.')
    note('  NEITHER WAS TUNED BACK. Freezing two named men against a scale change would mean special-')
    note('  casing them, which is the opposite of what "values mean" asks for.')
    line('discipline', 'parity green (worst 0.365 of 0.5) · 93 tests green · harness all nine in band · r71 gauge constants unchanged, summit pins EXACT (GSW ’17 OFF 99 · Pistons ’04 DEF 99) · suite 0/7 both bases', 'full battery', true)
    note('THE DEVIATION TAX LAW: one band broke, the OPPOSITE way from r85. hunt’s BLIND penalty went')
    note('  too SHALLOW (-0.25, band floor -0.30) while its oracle was healthy at +0.63 — so the tax')
    note('  went UP, 3.60 -> 3.72, and it reads random -0.35 / oracle +0.58. All nine in band. RECORDED:')
    note('  crash def glass now sits EXACTLY on the edge at random -0.30. It PASSES and was left alone;')
    note('  moving a constant that is inside its band would be tuning for taste, not for the law.')
    line('WINS GATE — the number this round was expected to move', 'r_def 0.501 -> 0.510 · r_off 0.502 -> 0.501', 'reported honestly either way; the 0.6 gate still FAILS', true)
    note('  The defensive side moved the right way and by very little: +0.009 on n=26. Honest reading —')
    note('  that is noise-adjacent, not evidence the round fixed the index. The offensive side drifted')
    note('  -0.001, which is the 120-team best-five churn from r85 still settling. NOT tuned toward.')
    note('MONOTONICITY, and this one deserves its own paragraph: mono68 reports 1 OFF violation, and it')
    note('  IS NOT THIS ROUND’S. Nazr Mohammed ’05 -> Shawn Kemp ’91 costs offRaw 0.0487 on a strictly-')
    note('  better swap. Measured on BOTH bases: -0.0487 before, -0.0487 after, byte-identical. recal_86')
    note('  moved perdef and only perdef (verified across all 21 attributes on all 10,000 cards), and')
    note('  offRaw reads no defensive attribute. What changed is WHICH fives the sampler draws, since it')
    note('  draws on OVR and 1,017 OVRs moved. This is the usage-economy flaw already named and left')
    note('  unfixed. Newly EXPOSED, not newly caused, and NOT hidden by reseeding.')
    note('ONE TEST PIN RE-ANCHORED, with its reason in tests/defense.test.ts: the 60/40 wall-vs-sieve')
    note('  spread 2.61 -> 2.46 (assertion floor 2.5 -> 2.4). STILL PENDING TOMER, and the CAUSE IS NEW:')
    note('  r76 and r81 moved WHO the elite defenders are; recal_86 moved the FLOOR under everyone else.')
    note('  The WALL pool (perdef >= 85) did not change by a single card, 351 -> 351, because the elite')
    note('  are pinned by their own absolute floors — while the sieve tier (perdef <= 35) went 525 ->')
    note('  346 and population sd 15.33 -> 14.90. The wall stood still and the sieve came up under it.')
    note('  This REVERSES r76’s recorded finding that "the population did NOT compress, so DRTG_COEF is')
    note('  NOT the lever". This time it did compress, for a reason the round intended. Still his call.')
    note('SELF-RESOLVED SINCE r85, worth recording: the ice scheme is back in band (it was 99/2000 =')
    note('  4.95% against a 5% floor) and "every scheme earns its slot" passes again. The pool moved')
    note('  under it a second time and carried it back. Nothing was tuned to achieve that.')
  },
  '85': () => {
    console.log(`${EOL}recal_85 — THE BLEND IS THE WHOLE OF OVR (his rulings, verbatim: "Kill breadth and the tax", then "Kill the band too")`)
    const V85 = Number((OVR.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    const V85b = Number((RATINGS.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    line('PIPELINE_VERSION', `shipped at 77 · current ${V85} / ${V85b}`, 'both 77', V85 === 77 && V85b === 77)
    note('OPTION B, as he ordered on the second ruling: all THREE shaping terms go, not just the two.')
    note('  The chain was  blend -> empty-volume tax -> breadth+fade -> offence cap -> top band -> clamp.')
    note('  The chain is now  blend -> offence cap -> clamp 99.  Nothing else touches an OVR.')
    src('the blend, whole and alone', OVR, /raw = max\(0\.4 \* p\['o_ovr'\] \+ 0\.6 \* p\['d_ovr'\], 0\.70 \* p\['o_ovr'\] \+ 0\.30 \* p\['d_ovr'\]\)/, "r83's blend, with nothing between it and the cap")
    src('the clamp reads the raw blend', OVR, /p\['ovr'\] = int\(min\(99, cap, round\(raw\)\)\)/, 'no band_ovr() in the clamp')
    line('empty-volume tax GONE', /raw -= min\(5\.0, 0\.06/.test(OVR) ? 'STILL PRESENT' : 'gone', 'gone', !/raw -= min\(5\.0, 0\.06/.test(OVR))
    line('breadth + fade GONE', /breadth = 4\.0 if solid >= 6/.test(OVR) ? 'STILL PRESENT' : 'gone', 'gone', !/breadth = 4\.0 if solid >= 6/.test(OVR))
    line('band_ovr() GONE', /def band_ovr/.test(OVR) ? 'STILL PRESENT' : 'gone', 'gone', !/def band_ovr/.test(OVR))
    line('OVR_KNEE / OVR_TOP GONE', /OVR_KNEE|OVR_TOP = /.test(OVR) ? 'STILL PRESENT' : 'gone', 'gone', !/OVR_KNEE|OVR_TOP = /.test(OVR))
    note('OVR_TOP, THE DEAD-CODE VERDICT the round asked for: recal_84 re-derived it to 97.10 and')
    note('  established it needed no further re-derivation because breadth faded above the knee. With the')
    note('  band killed, band_ovr() was the ONLY consumer of OVR_TOP and OVR_KNEE, and band_ovr() was')
    note('  called from exactly one place — the OVR clamp. Both constants and the function are therefore')
    note('  DEAD CODE, and they are REMOVED rather than left to rot and mislead a later round into')
    note('  re-deriving a number nothing reads. Verified by search: the only surviving mention of the name')
    note('  in the pipeline is the comment that records this. `_tops` survives as a diagnostic — it now')
    note('  collects the pure blend, so the run still prints the top of the board and drift stays visible:')
    note('  "OVR blend top 97.10 — no band, no anchor: OVR is the blend, capped and clamped".')
    const above = PLAYERS.filter((p) => p.ovr > Math.max(p.o_ovr, p.d_ovr))
    const below = PLAYERS.filter((p) => p.ovr < Math.min(p.o_ovr, p.d_ovr))
    line('cards printing ABOVE their better end', `${above.length} (was 142)`, '0', above.length === 0)
    line('cards printing BELOW their weaker end', `${below.length} (was 192)`, '0', below.length === 0)
    note('WHICH TERM OWNED WHICH, measured before each was killed rather than assumed:')
    note('  - the empty-volume TAX was the SOLE cause of all 192 below-both.')
    note('  - BREADTH (with the tax) accounted for the 142 above-both, Embiid ’26 among them.')
    note('  - with those two gone the BAND was the only remaining cause of an above-both, and it did it')
    note('    for exactly fourteen. He was shown that list and ruled "Kill the band too".')
    const emb = g("Joel Embiid '26")
    line("the case that opened the round — Embiid '26", `OVR 92 -> ${emb.ovr} (O ${emb.o_ovr} D ${emb.d_ovr})`, 'inside his own two ends', emb.ovr <= Math.max(emb.o_ovr, emb.d_ovr) && emb.ovr >= Math.min(emb.o_ovr, emb.d_ovr))
    note('THE FOURTEEN the band was lifting above their better end — before -> after, all measured now:')
    const F14: [string, number][] = [["LeBron James '08", 97], ["LeBron James '09", 98], ["LeBron James '10", 99], ["LeBron James '11", 97], ["LeBron James '12", 98], ["LeBron James '13", 98], ["Giannis Antetokounmpo '19", 97], ["Giannis Antetokounmpo '20", 98], ["Giannis Antetokounmpo '21", 97], ["Giannis Antetokounmpo '22", 98], ["Giannis Antetokounmpo '23", 98], ["Michael Jordan '89", 97], ["Michael Jordan '90", 97], ["Michael Jordan '91", 96]]
    let f14ok = 0
    for (const [n, was] of F14) {
      const q = g(n)
      const ok = q.ovr <= Math.max(q.o_ovr, q.d_ovr)
      if (ok) f14ok++
      line(`  ${n}`, `OVR ${was} -> ${q.ovr}  (O ${q.o_ovr} D ${q.d_ovr})`, 'at or below his better end', ok)
    }
    line('the fourteen, all inside their own ends', `${f14ok}/14`, '14/14', f14ok === 14)
    const top20 = [...PLAYERS].sort((a, b) => b.ovr - a.ovr).slice(0, 20)
    const distinct = new Set(top20.map((p) => p.ovr)).size
    note('WHAT WAS GIVEN UP, stated plainly. The band’s stated job was separating the top of the board.')
    line('top twenty: distinct OVR values', `${distinct} (was 4: 99/98/97/96)`, 'the blend still separates, but by less', distinct >= 3)
    line('top twenty: OVR range', `${top20[0].ovr}..${top20[19].ovr} (was 99..96)`, 'a 2-point spread where the band bought 3', true)
    note('  THE HONEST READING: the blend DOES still order the top — underneath the printed integers it')
    note('  runs LeBron ’10 at 97.10 down to Jordan ’91 at 95.00, a 2.10-point spread the band used to')
    note('  stretch onto 6. Rounding to integers is what costs the resolution, not the blend. The board is')
    note('  one value flatter at the summit than it was, and that is the price of his ruling: a card can')
    note('  never print above its better end, and the summit pays for it in ties. He was shown the')
    note('  fourteen before the second ruling and ruled anyway. A cost, recorded — not a defect.')
    note(`  the summit now: ${top20.slice(0, 6).map((q) => `${q.ovr} ${q.name}`).join(' · ')}`)
    line('OVR 99 is now unreachable', `max OVR ${Math.max(...PLAYERS.map((q) => q.ovr))}`, 'the blend maxes at 97.10, so 97 is the ceiling', true)
    note('  NAMED, NOT FIXED: nothing in the pool reaches 99 any more, because only the band ever carried')
    note('  a card there. Receipt "ovrtop" and recal_40’s ceiling pin both MISS for this reason and are')
    note('  annotated as superseded. If he wants the 99 back it is a RULING (re-scale the blend), not a')
    note('  tuning — and it cannot be had together with "never above your better end" on this arithmetic.')
    line('FOOTPRINT — both directions, as standing practice requires', '1,223 rose · 637 fell · 8,140 unchanged · mean +0.34', 'the tax removal lifts more men than the band removal drops', true)
    note('  UP (the tax): Walter Davis ’91 56->62, Maxey ’26 70->76, Cassell ’06 62->68, Randle ’23 66->72,')
    note('    Jaylen Brown ’22 66->72, Isiah ’88 60->66 — every +6 is a heavy-load, low-efficiency scorer')
    note('    who was paying the full 5-point empty-volume tax. Largest rise is +6, and 20 men take it.')
    note('  DOWN (breadth, then the band): Anthony Davis ’23 and McHale ’87 both 94->89 (-5) are the deepest')
    note('    falls — O 81 / D 95 sheets that cleared six breadth groups AND sat above the 93 knee, so they')
    note('    paid twice. Then AD ’14/’16/’22/’24/’25, Horford ’17, Bam ’25, Barkley ’91/’93, Griffin ’15/’19,')
    note('    Penny ’96/’97, all -4. The class is exactly the all-around big, which is what breadth paid.')
    line('archetype changes', '11 seasons', 'small and explicable', true)
    note('  McGrady ’01/’05 Co-star -> All-around star (tax refund); Pierce ’03 All-around star -> Co-star')
    note('  (breadth refund); Porzingis ’17/’20 -> Unicorn; six Unclassified sheets clear into Balanced,')
    note('  which is why receipt 55’s Unclassified count now reads 11 -> 2 instead of 11 -> 8.')
    line('tier crossings at the 90/80/70/60 cuts', '620 cards', 'a real but bounded re-tiering', true)
    line('teams fielding a DIFFERENT starting five', '120 of 1,299 (9.2%)', 'OVR-max boards move when OVR moves', true)
    note('FIVE OF THEM, named, with the man in and the man out (scripts/fives85.ts prints all 120):')
    note('  2024 Boston Celtics    IN Jaylen Brown ’24 (66->71)      OUT Jrue Holiday ’24 (67->67)')
    note('  2026 Toronto Raptors   IN RJ Barrett ’26 (60->65)        OUT Collin Murray-Boyles ’26 (64->64)')
    note('  1984 Chicago Bulls     IN Quintin Dailey ’84 (54->59)    OUT Mitchell Wiggins ’84 (57->57)')
    note('  1987 New York Knicks   IN Gerald Wilkins ’87 (52->57)    OUT Kenny Walker ’87 (55->55)')
    note('  1980 Golden State W.   IN Purvis Short ’80 (58->63)      OUT Sonny Parker ’80 (60->60)')
    note('  ONE MECHANISM RUNS ALL FIVE: the man coming IN rose and the man going OUT did not move. Every')
    note('  one of the five arrivals is a volume scorer who had been paying the empty-volume tax; the men')
    note('  they displace are role players who never paid it. The boards did not get better or worse —')
    note('  they stopped being sorted by a term he has now removed.')
    note('  Boston ’24 is the one with a visible engine consequence: its suite DEF reading moves 69 -> 57')
    note('  because Holiday leaving the five takes his defence with him. Reported, not corrected — the')
    note('  five is chosen by OVR, and OVR is now exactly what he ruled it should be.')
    line('discipline', 'parity green · mono68 500/500 (OFF 0 / DEF 0 violations) · 93 tests green · r71 gauge constants unchanged, summit pins EXACT (GSW ’17 OFF 99 · Pistons ’04 DEF 99) · suite 0/7 both bases', 'full battery', true)
    note('THE DEVIATION TAX LAW, APPLIED AS WRITTEN: two bands BROKE and the taxes moved to meet them.')
    note('  hunt oracle +0.46 and crash-def oracle +0.44 both fell under the +0.5 floor when the ovr>=55')
    note('  pool moved under them. Re-ratified THROUGH THE HARNESS only: TAX.hunt 3.74 -> 3.60 (oracle')
    note('  +0.52, random -0.57) and TAX.crashDef 0.72 -> 0.46 (oracle +0.51, random -0.44). All nine')
    note('  tactics back in band. NOTHING in the OVR chain was touched to hold a band up — that is the')
    note('  direction the law runs, and it ran that way.')
    note('WINS GATE, reported honestly: r_off 0.502 / r_def 0.501 against the 0.6 gate — UNCHANGED to')
    note('  three decimals from the pre-round reading. It still FAILS, as it has since r77. The team')
    note('  engine reads ATTRS, not OVR, so the only route by which this round could move it is the 120')
    note('  changed fives, and 26-team Pearson did not feel them. Not tuned toward, not claimed as')
    note('  progress. The eight-team anchor suite also holds at 0/7 on both bases, exactly as at r84.')
    note('ONE TEST PIN RE-ANCHORED, with its reason recorded in tests/engine.test.ts: Curry ’16 91 -> 89.')
    note('  He pays NOTHING to the empty-volume tax (efficiency 98) and his blend of 88.70 sits below the')
    note('  old 93 knee where the band was identity — so the entire 2 points was BREADTH: he cleared five')
    note('  of the seven groups, worth +2.0 at full fade value. He now prints his blend and nothing else.')
    note('SUPERSESSIONS WRITTEN, NEVER DELETED: recal_17 (breadth fade), the ovr2 ratification, recal_37')
    note('  (three of its four modifiers), recal_40 (the ceiling pin), the "ovrtop" band round IN FULL,')
    note('  and recal_84 (the constant it re-derived) all carry a superseded-by-85 note quoting both of')
    note('  his rulings. Their MISSes in this ledger ARE the supersession showing, which is what the')
    note('  ledger header says a MISS means. One knock-on outside the OVR chain is recorded under')
    note('  recal_75: the ice scheme fell to 99/2000 = 4.95% against that round’s own 5% share floor —')
    note('  one call short, caused by the pool moving. Measured and reported, NOT tuned.')
  },
  '84': () => {
    console.log(`${EOL}recal_84 — THE OVR CEILING DRIFT, FIXED (his ruling: "Fix the OVR ceiling drift"; completes the observation recorded in receipt 83)`)
    const V84 = Number((OVR.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    line('PIPELINE_VERSION', `shipped at 76 · current ${V84}`, '>= 76', V84 >= 76)
    src('the re-derived anchor', OVR, /OVR_KNEE, OVR_TOP = 93\.0, 97\.10/, '96.50 -> 97.10')
    note('SUPERSEDED BY recal_85 — the constant this round re-derived is GONE. His rulings, verbatim:')
    note('  "Kill breadth and the tax", then "Kill the band too". With the band killed, OVR_TOP and')
    note('  band_ovr() consumed nothing at all, so recal_85 removed them as DEAD CODE rather than leave')
    note('  a live-looking constant rotting in the file. recal_84 was correct when it shipped and its')
    note('  derivation is still the reason 97.10 was the right number; the round it served no longer runs.')
    note('THE DERIVATION, done from the pool rather than typed: the constant’s doctrine is "top set to the')
    note('highest raw the blend actually produces so the best card lands ON 99". THE READING TAKEN — and')
    note('the wording does admit two — is the value band_ovr CONSUMES, i.e. after the empty-volume tax')
    note('and the breadth term, before the band and the cap. That is the only reading under which the')
    note('constant does its job, since its entire function is to map band_ovr’s input onto 99; the raw')
    note('blend before tax and breadth is not what the band ever sees. Measured maximum over all 10,000')
    note('cards: LeBron James ’10 at 97.1000 exactly. The run now prints "0 cards above the anchor".')
    note('LINEAGE, so the record shows why an OVR constant moved for a defensive round’s reason: the drift')
    note('arrived with recal_80’s DEF display re-solve (multiplier 1.03 -> 1.1305), which lifted every')
    note('d_ovr below the 93 knee and with it the OVR blend that reads d_ovr. recal_83 printed it; this')
    note('round fixes it. Nothing in recal_83’s own ruling caused it.')
    line('THE FOUR CARDS THAT WERE CLAMPED ABOVE THE ANCHOR', "LeBron '10 99 -> 99 · Giannis '24 99 -> 99 · LeBron '13 99 -> 98 · Giannis '25 99 -> 98", 'the compression at the top is released', true)
    note("  Four more printed 99 without being above it and now separate too: LeBron '12, LeBron '09,")
    note("  Giannis '22 and Giannis '20, all 99 -> 98.")
    line('cards at OVR 99', '8 -> 2', 'the summit is a summit again rather than an eight-way tie', true)
    note('  ON "EXACTLY ONE AT 99": it is TWO, and they tie honestly rather than being forced apart.')
    note('  LeBron ’10 lands ON 99 by construction (his raw IS the anchor). Giannis ’24 sits at raw 96.90,')
    note('  which the re-derived band maps to 98.71 and rounding takes to 99. That is a genuine tie at the')
    note('  top, not a second clamp — no card is above the anchor now — so it is reported, not separated.')
    line('FOOTPRINT — tiny and concentrated at the very top, as expected', '30 cards moved, ALL by exactly -1 · 0 rose · 9,970 unchanged · mean -0.003 · nothing moved 2 or more', 'the band only bites above the 93 knee, so only the elite tier can feel it', true)
    note('  Every mover is an all-time card: Giannis ’18-’25, LeBron ’09/’12/’13, Jordan ’88/’89/’90, Durant')
    note('  ’13/’17/’18/’19, Malone ’93/’97/’98/’00, Garnett ’03/’07, Shaq ’02/’03, Duncan ’07, Kawhi ’16/’17,')
    note('  Jokic ’25, SGA ’26, Embiid ’23. Below the knee not one card moved.')
    note('THE OTHER-ANCHORS AUDIT, since I was in there — three constants share this doctrine:')
    note('  OVR : anchor 96.50, measured 97.10, FOUR cards above — a cluster compressed together. FIXED.')
    note('  OFF : anchor 106.36, measured 110.08, ONE card above. LEFT, deliberately: the doctrine’s own')
    note('        sentence is "a future outlier past them simply pins at 99, which is what a ceiling is')
    note('        for". One card past the anchor IS that sanctioned case; four is not. The gap is wide')
    note('        (3.72) but the distribution is empty between 106.36 and 110.08, so exactly one man is')
    note('        clamped and nobody is tied to him. Named here so it is a decision, not an oversight.')
    note('  DEF : anchor 107.55, measured 107.56 — a 0.01 rounding gap, not drift. It was re-derived in')
    note('        recal_80 and is still true. LEFT untouched.')
    line('discipline', 'parity green · mono 500/500 · harness all nine in band with NO re-ratification needed · r71 gauge constants unchanged and summit pins EXACT (GSW ’17 OFF 99 · Pistons ’04 DEF 99) · suite 0/7 both bases', 'full battery', true)
    note('  Wins gate against r_off 0.502 / r_def 0.501: unchanged at 0.502 / 0.501. The team engine')
    note('  reads attrs, and this round moved only a display band, so nothing there could shift. No test')
    note('  pin needed re-anchoring — the near() anchors gate on o_ovr/d_ovr, which did not move.')
    note('  93 tests green.')
  },
  '83': () => {
    console.log(`${EOL}recal_83 — OVR IS THE BETTER OF TWO ROLE READINGS (his ruling: "OVR = bigger of ((OFF*0.7 + DEF*0.3), (OFF*0.4 + DEF* 0.6))")`)
    const V83 = Number((OVR.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    line('PIPELINE_VERSION', `shipped at 75 · current ${V83}`, '>= 75', V83 >= 75)
    note('PREMISE — OVR WAS ALREADY THIS SHAPE, which is worth stating before the diff is read.')
    note('  BEFORE: raw = max(0.4*o_ovr + 0.6*d_ovr, 0.75*o_ovr + 0.25*d_ovr)   [recal_40, "THE CORE"]')
    note('  AFTER : raw = max(0.4*o_ovr + 0.6*d_ovr, 0.70*o_ovr + 0.30*d_ovr)')
    note('  His defence-led branch is EXACTLY the one already shipped. Only the offence-led branch')
    note('  moves, 0.75/0.25 -> 0.70/0.30. It is not a replacement of some other construction: OVR has')
    note('  been the better-of-two-role-readings since recal_40, and marginal value has been out of it')
    note('  since recal_37. What follows it is unchanged — empty-volume tax, breadth, the offence cap,')
    note('  and the top-band rescale all still apply on top.')
    src('the law, in the pipeline', OVR, /raw = max\(0\.4 \* p\['o_ovr'\] \+ 0\.6 \* p\['d_ovr'\], 0\.70 \* p\['o_ovr'\] \+ 0\.30 \* p\['d_ovr'\]\)/, 'his weights, verbatim')
    note('  THE ALGEBRA, as ordered: the branches cross exactly where o_ovr == d_ovr, since')
    note('  0.7o + 0.3d = 0.4o + 0.6d reduces to 0.3o = 0.3d. So the rule reads "0.7*hi + 0.3*lo" when')
    note('  offence is the stronger end and "0.6*hi + 0.4*lo" when defence is. It is monotone in both')
    note('  inputs and still tilted toward offence. The consequence is exact and one-directional: only')
    note('  men whose offence exceeds their defence move, and they lose precisely 0.05*(o_ovr - d_ovr).')
    line('FOOTPRINT, two-directional', '2,613 fell · 7 rose · 7,380 unchanged · mean -0.33 · |move| >= 5: ZERO, >= 3: 33, >= 2: 657', 'bounded by construction — the largest possible loss is 0.05 x the o/d gap', true)
    note('  THE TWENTY LARGEST FALLS ARE THE LOPSIDED SCORERS, exactly as expected: Adrian Dantley')
    note("  '80-'87 (all -3, O 90-93 on D 33-38), Damian Lillard '17/'18/'23, DeMar DeRozan '20/'21,")
    note("  Devin Booker '18/'19/'20, Jalen Brunson '23-'26, Alex English '87. Elite offence, no")
    note('  defence — the men his weighting is aimed at. Nobody fell more than 3.')
    note('  THE SEVEN RISES ARE A BREADTH INTERACTION, not a contradiction: SGA ’25/’26, Jokic ’25/’26,')
    note("  Durant '13/'19, Malone '93 all sit just under the 93 breadth knee, and the breadth FADE pays")
    note('  more the further below it you are. Losing ~1.1 of raw bought them ~1.5 of breadth back, so')
    note('  they print +1. The raw score fell for every one of them; only the printed OVR rose.')
    line('ARCHETYPE CONSEQUENCE — almost none', "1 card: Blake Griffin '13 All-around star -> Co-star (OVR 82 -> 81, crossing that rule's own 82 gate)", 'the ovr-gated rules (Unicorn, All-around star, the two-way family) barely notice', true)
    line('BEST-FIVE CONSEQUENCE — the one that changes the game he plays', '69 of 1,255 wheel teams (5.5%) field a different best five', 'measured, not assumed', true)
    note("  Three named: Golden State '80 (Purvis Short -> Sonny Parker), Washington '80 (Roger Phegley")
    note("  -> Kevin Grevey), San Antonio '82 (Mike Mitchell -> Mark Olberding). Every example is a")
    note('  bench-tier swap between the 4th and 5th men, not a star changing hands — which is what the')
    note('  bounded -3 ceiling predicts.')
    line('the auction Machine', '174 cards change tier value at a $16 ceiling', 'it prices by OVR tiers, so the tier boundaries move with them', true)
    line('team OVR in the db is UNAFFECTED, as expected', 'TeamDb reads Math.round((g.off + g.def) / 2) — the gauge composite, not card OVR', 'confirmed in source', true)
    line('discipline', 'parity green · mono 500/500 (his formula is monotone in both inputs, and it holds) · harness re-ratified (crashOff .55->.42, crashDef .95->.72, all nine in band) · summit pins EXACT (GSW ’17 OFF 99 · Pistons ’04 DEF 99) · suite 0/7 both bases', 'full battery', true)
    note('  Wins gate against r_off 0.510 / r_def 0.492: now 0.502 / 0.501 — offence a touch down,')
    note('  defence a touch up, both flat. One pin re-anchored with its reason: Curry ’16 OVR 92 -> 91')
    note('  (O 95 / D 73, so he gives back 1.1 by the rule). Cards at OVR 99: 9 -> 8.')
    note('  RECORDED, NOT FIXED — a drift this round did not cause: the OVR band anchor OVR_TOP is 96.50')
    note('  while the pool now produces a raw top of 97.10, so 4 cards sit above the anchor and clamp at')
    note('  99 together. By that constant’s own doctrine ("top set to the highest raw the blend actually')
    note('  produces so the best card lands ON 99") it wants re-deriving to 97.10. The drift came in with')
    note('  r80’s DEF display re-solve, not with his ruling, so I left it rather than fold someone else’s')
    note('  correction into this round’s attribution. It is a one-line change whenever he wants it.')
  },
  '82': () => {
    console.log(`${EOL}recal_82 — THE RIMPROT BAR, GENUINELY GRADED (his ruling: "So lets change the bar. 0.4 != 1. The more the better")`)
    const V82 = Number((OVR.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    line('PIPELINE_VERSION', `shipped at 73 · current ${V82}`, '>= 73', V82 >= 73)
    src('the bar is now the reputation itself', RATINGS, /_w53 = min\(1\.0, max\(0\.0, r\['drep'\]\)\) if r\['drep'\] > 0\.05 else 0\.0/, 'w = drep — monotone, no plateau, only a unanimous vote earns the whole band')
    note('HIS QUESTION, ANSWERED IN ONE LINE: Noah ’09 read 97 because a 0.43 reputation cleared the old')
    note('0.30 saturation point, so min(1, 0.43/0.30) = 1.00 bought him the IDENTICAL full membership a')
    note('unanimous DPOY gets — and once inside, the band top is uncapped, so nothing held him back.')
    line("Noah '09, the card he asked about", 'rimprot 97 -> 92 (drep 0.43) · DEF 78 -> 76', 'a fringe vote now buys 43% of the band, not all of it', true)
    note('  HIS INSTINCT UNLOCKED THIS ROUND. My first pass stopped it as infeasible, and that stop was')
    note('  answering a harder question than the round asks: I required a single ceiling to produce the')
    note('  whole ordering by itself. Composed — grade first, cap second — the grading does the work on')
    note('  Noah and the ceiling never has to bind on him at all. The stop was mine, and it was wrong.')
    line('THE FIVE ANCHORS ALL HOLD', "Gobert '19 97 · Wembanyama '24 97 · Mutombo '97 98 · Duncan '03 98 · Ben Wallace '04 98", 'every one is drep 1.00, so his rule leaves them untouched — verified, not assumed', true)
    line('the three targets', "Bobby Jones '82 96 HOLDS · Caldwell Jones '82 97 (target 94-96) · Noah '09 92 (target 89-91)", '6 of 8 — the two misses are ONE POINT each, and both need the ceiling that cannot ship', false)
    note('  WHY THE EVIDENCE CEILING DID NOT SHIP, and it is his own law that decides it: the ceiling is')
    note('  the only thing that can bring Caldwell Jones down (he is drep 1.00, so grading cannot touch')
    note('  him). Measured both ways — with DBPM it works (he lands 96, 7 of 8 hold); with blk% alone it')
    note('  CANNOT, because Caldwell Jones ’82 and Ben Wallace ’04 sit at the IDENTICAL block percentile')
    note('  0.67 within their own voted pools, while one must be capped to 96 and the other must stay at')
    note('  97+. Blocks alone cannot separate them; only DBPM does (1.0 against 3.3). recal_81 removed')
    note('  DBPM from rim protection on his ruling, so the ceiling has no lawful evidence term. Held.')
    note('  I reported the opposite last time — that the DBPM collision "never becomes the deciding')
    note('  question". Composed, it IS the deciding question. Corrected.')
    line('THE SATURATION BROKE ANYWAY', 'cards at rimprot 99: 54 -> 12', 'cause 2 largely fixed by cause 1 — the graded bar spread the top out on its own', true)
    line('FOOTPRINT, two-directional', '353 cards fell, 0 rose, 9,647 unchanged · mean -0.09 · worst -7 · d_ovr moved on 271', 'all falls, by construction: a graded bar can only remove membership', true)
    note('  Of 777 voted bigs, 127 sit at drep 1.00 and are untouched; 44% move at all. Twenty largest')
    note('  falls, all -6 or -7, all fringe votes: Ewing ’95/’94/’93/’91 (drep 0.33-0.51), Shaq ’96/’97/')
    note('  ’05/’06 (0.28-0.42), Mutombo ’92/’05, Gilmore ’80/’81, Tree Rollins ’80/’88, Manute Bol ’89/’90,')
    note('  McHale ’81, Robinson ’01, Hibbert ’10, Mourning ’95.')
    note('  THE COLLATERAL I WOULD NAME, since he asked for it plainly: band membership is now PURELY')
    note('  reputational. Manute Bol ’89 blocks 10.8% of shots — the highest rate in the database — and')
    note('  falls to 92 on a 0.41 vote, while Garnett ’05 blocks 2.6% and keeps 99 on a unanimous one.')
    note('  His rule is right that 0.4 != 1; the gap it exposes is that evidence still cannot EARN')
    note('  membership, only reputation can, and a ceiling caps but never lifts. If he wants a historic')
    note('  shot-blocker to reach the band without the votes, that needs a floor, not a ceiling — a')
    note('  separate ruling, named here and not taken.')
    note('PERDEF, OBSERVATION ONLY (not changed, as scoped): 67% of its 1,474 voted men sit between 0.30')
    note('  and 1.00, i.e. saturated at full membership under its own 0.30 bar — the same shape he just')
    note('  ruled out of rimprot. A follow-up if he wants it.')
    line('discipline', 'parity green · harness re-ratified (hunt 3.70->3.74, all nine in band) · r71 DEF_MID 109.12->109.16, summit pins EXACT (GSW 99 / Pistons 99) · suite 0/7 both bases · wall-vs-sieve spread 2.61, unmoved', 'full battery', true)
    note('  Wins gate against r_off 0.510 / r_def 0.494: now 0.510 / 0.496 — r_def up a touch, r_off flat.')
    note('  mono 499/500: the ONE violation is the pre-existing usage-economy pair (Duncan ’15 dominates')
    note('  Jim Petersen ’87 yet lowers offRaw), identical at v71/v72/v73 — not this round. 92 tests green.')
  },
  '81': () => {
  ...
  '75': () => {
    ...
    for (const s of live75) {
      ...
      line(`  ${s.key}`, `blind EV ${b.toFixed(2)} · best call ${bestCnt[s.key]}/${N75} = ${share.toFixed(2)}% · fit p10 ${f[200].toFixed(0)}/med ${f[1000].toFixed(0)}/p90 ${f[1800].toFixed(0)}`, 'blind in [-1.5,-0.3] and a real share of the oracle', legal)
    }
    ...
    note('KNOCK-ON FROM recal_85, REPORTED NOT TUNED: ice now MISSES this round\u2019s own share floor at')
    note('  99/2000 = 4.95% against a floor of 5% \u2014 ONE call short of legal. Killing the tax, breadth and')
    note('  the band moved 1,860 OVRs and with them the ovr>=55 pool these 2,000 matchups are drawn from,')
    note('  so ice lost a handful of matchups where it had been the oracle-best call. Nothing about ice')
    note('  itself changed. The r59 LAW still holds on the scheme row (random -0.78, oracle +0.51, with')
    note('  TAX.scheme unchanged at 0.90) \u2014 the law is measured over the WHOLE set, and it is the law')
    note('  that governs. This is a within-round acceptance band from r75, not the r59 law, and it is one')
    note('  call from passing. NOT tuned: bending a scheme constant to buy back one matchup is exactly')
    note('  what the report-don\u2019t-tune rule exists to stop. Recorded for a ruling if he wants ice wider.')
  ...
  '68': () => {
    ...
    note('ONE VIOLATION SINCE recal_86, AND IT IS NOT recal_86\u2019S. The swap is Nazr Mohammed \u201905 ->')
    note('  Shawn Kemp \u201991 in a five with Cardinal \u201904 / Gminski \u201988 / Ross \u201919 / Nance Jr. \u201918:')
    note('  offRaw 129.543 -> 129.494, a drop of 0.0487 on a strictly-better man. MEASURED ON BOTH BASES')
    note('  AND IDENTICAL: -0.0487 before the round and -0.0487 after. recal_86 moved perdef and nothing')
    note('  else — no offensive attribute moved for any card in the pool — so offRaw for this five is')
    note('  byte-identical across the round. What changed is WHICH fives the sampler draws, because it')
    note('  draws on OVR and 1,017 OVRs moved. This is the USAGE-ECONOMY FLAW already named and left')
    note('  unfixed: teamOffense routes surplus usage to creators, so a strictly-better man can take')
    note('  touches that were worth more elsewhere. Newly EXPOSED, not newly caused. Still not fixed')
    note('  here — fixing the usage economy is its own round, and forcing 500/500 by reseeding would be')
    note('  hiding it. The seed is left where it is precisely so the flaw stays visible.')
  ...
  '62': () => {
    ...
    note('60 SINCE recal_86 (57 -> 60). He is a tracked defender with a positive 6ft+ diff who used to')
    note('  be read at the bottom of a percentile and is now read on an absolute line, which is the')
    note('  whole point of that round. The MECHANISM this receipt guards — steals no longer buy perdef —')
    note('  is untouched; what moved is the scale the tracked term is expressed on. Recorded, not tuned.')
