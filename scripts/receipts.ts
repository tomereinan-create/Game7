...
const ROUNDS: Record<string, () => void> = {
  ...
  '79': () => {
    console.log(`${EOL}recal_79 — BALLSEC KEYED ON AST/TOV: PREMISE ACCURATE, STOPPED ON ITS OWN RED LINE (design-side round "71"; our 71 was the gauge scale)`)
    const V79 = Number((OVR.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    line('PIPELINE_VERSION', `untouched · current ${V79}`, 'STOPPED — nothing shipped, cards at v70', V79 >= 70)
    src('OUR formula, and the round quotes it EXACTLY', RATINGS, /0\.55\*Padj\(_bsec\(r\)\) \+ 0\.45\*Pa\['tov_pct'\]/, 'sc(1 - (0.55 P_ratio + 0.45 P_tov)) with denominator usg + 0.5*ast')
    note('  PREMISE ACCURATE — the first time in five rounds. No stale constants, no misread series, no')
    note('  wrong team. Both parts describe our real code, and the defect is real: the raw-TOV side is')
    note('  blind to assists, and an assist that produces a made shot ends the possession outright.')
    line('THE UNNAMED CARD, identified', "John Stockton '97 — 10.5 ast on 3.0 tov, exactly the 3.50 ratio quoted", 'his shipped ballsec is 29', true)
    note('  THE SEVEN NAMED MEN, real AST/TOV, ballsec under BOTH parts and under part 2 alone:')
    note('    player                AST/TOV  base   both   part2')
    note("    John Stockton '97       3.43     29   +19     +6")
    note("    John Stockton '90       4.13     40   +23    +12")
    note("    Chris Paul '09          3.74     70   +12    +11")
    note("    Steve Nash '07          3.06     36   +14     +6")
    note("    Magic Johnson '90       3.14     50   +14     +8")
    note("    Magic Johnson '89       3.15     52   +13     +8")
    note("    James Harden '19        1.52     61    +2     +1")
    note("    Russell Westbrook '17   1.91     59    +5     +3   <== RED LINE")
    note("    Michael Jordan '89      2.25     90    +2     +1")
    line('the proof the round asked for', 'corr(AST/TOV, ballsec move) = +0.704 over 10,000 cards', 'high ratio rose — the mechanism does exactly what it claims', true)
    line('THE ROUND CONTRADICTS ITSELF ON MAGIC — resolved from data', "Magic '89 AST/TOV 3.15 and '90 3.14 — BOTH above 2.5", "the round predicts '90 rises AND lists '89 as a control under 2.5; he is not under 2.5, and both rise", true)
    line('RED LINE BREACHED, and by BOTH variants', "Westbrook '17 moves +5 with both parts and +3 with part 2 alone; the round's limit is 2", 'STOP — this is r56’s protection and r56 answered a real complaint', false)
    note('  DIAGNOSIS, precise and falsifiable: the round’s credit floor is 1.5 and Westbrook sits at')
    note('  1.91, so he NECESSARILY takes credit 0.16 — the round assumed he would be "near credit 0".')
    note('  Because his raw-TOV percentile is far worse than his ratio percentile, even 16% of the weight')
    note('  shift moves him 3. Part 1 (the 0.8*ast denominator) adds the other 2, and part 1 is NOT keyed')
    note('  on AST/TOV at all — so the round’s own safeguard does not cover its own first part.')
    note('  RECORDED, NOT APPLIED: a credit floor of 2.0 zeroes Westbrook while costing Stockton ’97 only')
    note('  0.06 of credit. That is a change to the spec, so it needs a ruling rather than my hand.')
    line('FOOTPRINT, two-directional (unprompted)', 'both parts: 3,359 rose, 4,797 FELL, 1,844 unchanged, mean +0.50 · OFF moved on 2,322 cards, max |move| 2', 'MORE CARDS FELL THAN ROSE — the round describes only rises', true)
    note('  ballsec is a WITHIN-SEASON percentile, so lifting the elite passers pushes everyone else down.')
    note('  The falls land on high-ratio LOW-VOLUME men, which is the opposite of the round’s intent:')
    note("  T.R. Dunn '87 -13 at AST/TOV 4.25, Luke Kornet '26 -8 at 4.67, John Konchar '22 -9 at 3.64.")
    note('  Their ratios are excellent but their assist COUNT is tiny, so the bigger denominator barely')
    note('  helps them while the field moves past. Part 2 alone is gentler: 2,210 rose, 978 fell, OFF max 1.')
    note('  scripts/ballsec79.py reproduces the premise and the red-line arithmetic without a pipeline run.')
  },
  '78': () => {
