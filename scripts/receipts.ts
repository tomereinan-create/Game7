...
const ROUNDS: Record<string, () => void> = {
  ...
  '76': () => {
    console.log(`${EOL}recal_76 — PERDEF'S TEAM-DEFENSE DOUBLE-COUNT: PREMISE PROVED, ROUND STOPPED ON ITS OWN CONTROLS (design-side round "69"; our 69 was the wheel law)`)
    const V76 = Number((OVR.match(/PIPELINE_VERSION = (\d+)/) ?? [])[1])
    line('PIPELINE_VERSION', `untouched · current ${V76}`, 'STOPPED — nothing shipped', V76 >= 68)
    note('PREMISE — CONFIRMED, and quantified against OUR pool (scripts/perdef76.py reproduces it):')
    src('the vector matches the round exactly', RATINGS, /PD  = dict\(drep=0\.366, dbpm=0\.192, teamd=0\.192, height_inv=0\.25\)/, 'our real PD vector IS 0.366 / 0.192 / 0.192 / 0.25')
    ...
    line('THE STOP', 'cards reverted to v68, no version bump, no relabel shipped', 'premise sound, predictions refuted — re-rule with corrected expectations', true)
